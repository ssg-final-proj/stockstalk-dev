...
redis_client_user = redis.StrictRedis(host=os.getenv('REDIS_HOST'), port=6379, db=1, decode_responses=True)
        ...
        "today_orders": [
            {
                ...
                "created_at": order.Order.created_at.strftime('%Y-%m-%d %H:%M:%S')
            }
